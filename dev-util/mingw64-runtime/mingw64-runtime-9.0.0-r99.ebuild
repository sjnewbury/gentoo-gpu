...
IUSE="headers-only idl libraries tools posix-threads"
RESTRICT="strip"
...
src_configure() {
	...
	local libraries
	if use libraries; then
		libraries=$(crt_use_with libraries)
	elif use posix-threads && ! just_headers; then
		libraries=--with-libraries=winpthreads
	else
...
src_install() {
	...
	if just_headers && use posix-threads; then
		cp -R mingw-w64-libraries/winpthreads/include/*.h ${ED}/usr/${CTARGET}/usr/include
	fi
