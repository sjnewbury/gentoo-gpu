...
if ! tc_is_offload ; then
	...
	LIBPATH=${TOOLCHAIN_LIBPATH:-${PREFIX}/lib/gcc/${CHOST}/${GCC_CONFIG_VER}/accel/${CTARGET}}
fi
...
if [[ ${PN} != kgcc64 && ${PN} != gcc-* ]] ; then
	if [[ ${PN} != offload-* ]] ; then
		...
		tc_version_is_at_least 4.6 && IUSE+=" cpu_flags_x86_avx debian-multiarch"
		tc_version_is_at_least 4.7 && IUSE+=" go"
 		tc_version_is_at_least 4.8 && IUSE+=" gold"
		...
		tc_version_is_at_least 7.0 && IUSE+=" offload-nvptx offload-mic posix-threads"
		...
		tc_version_is_at_least 10 && IUSE+=" offload-rocm"
...
if [[ ${PN} == offload-gcc-nvptx ]] ; then
	DEPEND+=" dev-util/nvidia-cuda-toolkit"
fi

if [[ ${PN} == offload-gcc-rocm ]] ; then
	DEPEND+="
		sys-devel/llvm[llvm_targets_AMDGPU]
		sys-devel/amdgpu-binutils-wrapper
		"
fi

if tc_has_feature offload-nvptx ; then
	DEPEND+=" offload-nvptx? (
				~sys-devel/offload-gcc-nvptx-${PV}:${SLOT}=
			)"
fi

if tc_has_feature offload-mic ; then
	DEPEND+=" offload-mic? ( sys-devel/offload-gcc-mic )"
fi

if tc_has_feature offload-rocm ; then
	DEPEND+=" offload-rocm? ( sys-devel/offload-gcc-rocm )"
fi
...
toolchain_src_prepare() {
	...
	tc_is_offload && ln -s "${WORKDIR}"/newlib-${NEWLIBV}/newlib "${S}"/newlib
}
...
setup_multilib_osdirnames() {
	...
	local sed_args=()
	if tc_version_is_at_least 4.6 && ! _tc_use_if_iuse debain-multiarch ; then
...
toolchain_src_configure() {
	...
	tc_is_offload && GCC_LANG+=",lto,fortran"
	...
	if _tc_use_if_iuse cpu_flags_x86_avx && tc_version_is_at_least 4.6 ; then
		case $(tc-arch) in
	...
	if is_crosscompile ; then
		# Clear extra LIBS environment var
		unset LIBS 

		# force disable systemtap, headers on host gets detected
		# then build tries to use target headers
		...
		case ${CTARGET} in
			...
			*-cygwin)
			...
			x86_64-*-mingw*|*-w64-mingw*)
				if _tc_use_if_iuse posix-threads ; then
		...
		if [[ -n ${needed_libc} ]] ; then
			...
			if tc_is_offload ; then
				confgcc+=(
					 ...
					 --with-toolexeclibdir="${LIBPATH}"
				)
			elif ! has_version ${CATEGORY}/${needed_libc} ; then
				confgcc+=(
					"${confgcc_no_libc[@]}"
					--disable-threads
					--without-headers
	...
	case ${CTARGET} in
		*-gnu*)
			...
			if in_iuse offload-mic; then
				if use offload-mic ; then
					offload_targets="${offload_targets},x86_64-intelmicemul-linux-gnu"
				fi
			fi
			if in_iuse offload-rocm; then
				if use offload-rocm ; then
					offload_targets="${offload_targets},amdgcn-amdhsa"
				fi
			fi
	...
	case $(tc-arch) in
		arm)
			...
			[[ ${CTARGET} == *iwmmxt-* ]] && arm_arch="iwmmxt"
			...
			if [[ ${CTARGET} == *iwmmxt-* ]] ; then
				confgcc+=( --with-abi=aapcs-linux )
			fi
			;;
		mips)
	...
	if in_iuse openmp ; then
		...
		tc_is_offload || confgcc+=( --disable-libgomp )
	fi
	...
	if tc_version_is_at_least 4.6 && [[ ${CTARGET} != nvptx-none ]] ; then
		confgcc+=( --enable-lto )
...
toolchain_src_install() {
	...
	if tc_version_is_at_least 4.3 ; then
		if ! tc_is_offload ; then
			...
			pax-mark -r "${D}${PREFIX}/libexec/gcc/${CHOST}/${GCC_CONFIG_VER}/accel/${CTARGET}/cc1"
			pax-mark -r "${D}${PREFIX}/libexec/gcc/${CHOST}/${GCC_CONFIG_VER}/accel/${CTARGET}/cc1plus"
		fi
	fi
