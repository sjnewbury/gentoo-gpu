...
if [[ ${PN} != kgcc64 && ${PN} != gcc-* && ${PN} != offload-* ]] ; then
	...
	IUSE+=" fixed-point"
	IUSE+=" cpu_flags_x86_avx debian-multiarch"
	IUSE+=" go"
	IUSE+=" gold"
...
setup_multilib_osdirnames() {
	...
	local sed_args=()
	if ! _tc_use_if_iuse debain-multiarch ; then
...
toolchain_src_configure() {
	...
	if _tc_use_if_iuse cpu_flags_x86_avx ; then
		case $(tc-arch) in
...
toolchain_src_install() {
	...
	if ! tc_is_offload ; then
		...
		pax-mark -r "${ED}/libexec/gcc/${CHOST}/${GCC_CONFIG_VER}/accel/${CTARGET}/cc1"
		pax-mark -r "${ED}/libexec/gcc/${CHOST}/${GCC_CONFIG_VER}/accel/${CTARGET}/cc1plus"
	fi
	if use test ; then
