...
inherit base python-single-r1 autotools multilib flag-o-matic toolchain-funcs ${GIT_ECLASS}
...
src_install() {
	...
	ebegin "Moving libGL and friends for dynamic switching"
		...
		for x in "${ED}"/usr/$(get_libdir)/lib{EGL,GL*,OpenVG,OpenCL}.{la,a,so*}; do
			if [ -f ${x} -o -L ${x} ]; then
	...
	if use opencl && use gallium; then
